...
- [ ] Permission tier (standard vs. elevated) determined from the user's Snowflake role/permissions after login
...
- **Auth**: Microsoft SSO (OAuth/OIDC). Permission tier derived from Snowflake role, not SSO groups. No local fallback.
...
| Permission tier from Snowflake role, not SSO groups | Snowflake is already the authoritative access control boundary; avoids maintaining a separate elevated-access SSO group | — Pending |
