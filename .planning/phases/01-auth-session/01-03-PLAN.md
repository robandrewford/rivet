...
must_haves:
  truths:
    ...
    - "When re-authentication is triggered, the current URL is preserved in sessionStorage before redirect"
    - "After re-authentication, the user returns to their exact prior URL restored from sessionStorage (full workflow state serialization deferred to Phase 2+ when workflow UI exists)"
  artifacts:
...
<objective>
Implement access_token refresh via refresh_token rotation and client-side session expiry detection with URL preservation.

Purpose: AUTH-04 (session expiry handling + state preservation). In Phase 1, "state preservation" means restoring the user's prior URL after re-authentication. Full workflow state serialization (DAG state, active node, form inputs) is deferred to Phase 2+ when the workflow UI exists.

Output: Automatic token refresh, graceful re-authentication on refresh failure with URL checkpointing via sessionStorage.
</objective>
...
1. Define `async function refreshAccessToken(token: JWT): Promise<JWT>` (import `JWT` type from `next-auth/jwt`):
   - POST to `https://login.microsoftonline.com/${tenantId}/oauth2/v2.0/token` (where `tenantId` is parsed from the issuer — see step 2 below) with:
...
2. Parse the tenant ID from the existing `AUTH_MICROSOFT_ENTRA_ID_ISSUER` env var (which is `https://login.microsoftonline.com/<TENANT_ID>/v2.0`):
   ```typescript
   const tenantId = process.env.AUTH_MICROSOFT_ENTRA_ID_ISSUER!.split('/')[3]
   ```
   No new env var needed. The issuer URL is already required by Plan 01-01.
...
3. Update the `jwt` callback to call `refreshAccessToken` when the token is expired:
   ...
   async jwt({ token, account }) {
     ...
     if (Date.now() < (token.expires_at - 5) * 1000) {
       return token
     }
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <what-built>
    ...
    7. session.permissionTier available in session (foundation for Phase 5 download enforcement)
  </what-built>
  <how-to-verify>
...
4. URL checkpoint uses sessionStorage (tab-scoped)
5. SessionProvider wraps the application in root layout
6. End-to-end auth flow verified by human (checkpoint)
</verification>

<success_criteria>
- Access token refreshes automatically when expired
- Refresh failure triggers re-authentication with URL preservation
- Current URL checkpointed to sessionStorage before redirect
