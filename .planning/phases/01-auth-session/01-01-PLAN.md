...
must_haves:
  ...
  artifacts:
    ...
    - path: "packages/community/src/middleware.ts"
  ...
  key_links:
    - from: "packages/community/src/middleware.ts"
...
@packages/community/src/app/middleware.ts — Currently exists here but must move to src/middleware.ts (Next.js requirement)
...
<task type="auto">
  ...
  <action>
Create `packages/community/src/middleware.ts` (Next.js requires middleware at the src root, NOT inside app/). If a file exists at `packages/community/src/app/middleware.ts`, delete it first.

1. Import `{ auth }` from `@/lib/auth`.
2. Export `default auth` as the middleware function using next-auth v5's callback pattern:
   ```typescript
   ...
   The middleware runs on every matched route. If `req.auth` is falsy (no session), redirect to the sign-in page.
...
IMPORTANT: Next.js only recognizes middleware at the project root or `src/` root, NOT inside `app/`. The file MUST be at `packages/community/src/middleware.ts`.
